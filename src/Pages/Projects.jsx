...
function Projects() {
  ...
  // Get unique term options or default ones
  const availableTerms = [...new Set(projects.map(project => project.term || '25/26'))].sort().reverse()
  if (availableTerms.length === 0) availableTerms.push('26/27', '25/26')
  if (!availableTerms.includes('26/27')) availableTerms.unshift('26/27')

  // Filter projects based on selected status AND term
  const filteredProjects = projects.filter(project => {
    const pTerm = project.term || '25/26'
    const matchTerm = pTerm === termFilter
          ...
          {loading ? (
            ...
            <>
              <div className="container">
                <div className="filter-controls" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', opacity: 1, pointerEvents: 'auto', transform: 'none' }}>
                  ...
                  <div className="term-selector" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <label style={{ fontWeight: '600', color: '#1e3c72' }}>Leastic Term:</label>
                    <select 
                      value={termFilter}
                      onChange={handleTermFilter}
                      style={{ padding: '10px 20px', borderRadius: '50px', border: '1px solid #e2e8f0', background: 'white', color: '#1e3c72', fontWeight: '500', cursor: 'pointer', outline: 'none', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}
                    >
              ...
              <div className="container" ref={projectsRef}>
                {filteredProjects.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
                    No projects found for the {termFilter} term.
                  </div>
                ) : (
